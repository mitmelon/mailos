...
**An open-source AI communication runtime — email is the first connector.**

Email was designed for humans to communicate. MailOS is designed for AI agents to communicate on behalf of humans — reading, negotiating, and reaching agreements over ordinary email, and only interrupting the owner when their judgment is actually required.

MailOS connects to a mailbox, listens continuously, and immediately starts working: it protects against phishing and prompt injection, classifies and responds to incoming mail, negotiates autonomously within a policy you set, learns from newsletters, and surfaces business opportunities from patterns across your inbox.

Built for the **Qwen Cloud Hackathon** (Track 3: Agent Society) — powered end-to-end by **Qwen**.
...
npm install
cp .env.example .env
# edit .env: set QWEN_API_KEY at minimum
npm start
```

Open `http://localhost:4000` in a browser. First run shows a one-time setup page to create your admin account; after that it's a normal login.
...
3. Open **API-KEY Management** and create a new key.
4. Paste it into `.env` as `QWEN_API_KEY`, or into **Settings → Qwen provider** in the running app — the latter takes effect immediately, no restart.

---

## Architecture

Message arrives via IMAP IDLE (instant push, not polling) → **Guardian** (heuristics + AI-assisted injection detection; blocked mail stops here) → **Classification** (Newsletter / Meeting / Invoice / Personal / Customer / Marketing / Spam / Scam) → fans out over the event bus to **Memory**, **Newsletter Intelligence**, **Opportunity**, and **Decision** in parallel → Decision assigns `archive` / `notify` / `reply_draft` / `ignore` / `requires_human` → **Negotiation** handles anything that needs an autonomous reply, policy-gated and round-capped.

Every agent is independent and communicates **only** through the internal event bus — publish/subscribe, never direct agent-to-agent calls. The bus runs on real BullMQ-backed Redis queues when `REDIS_URL` is set, and an in-process equivalent otherwise — both paths run the identical pipeline.

### The pipeline agents

| Agent | Responsibility |
|---|---|
| **Guardian** | Blocks phishing, malicious attachments, and prompt injection — heuristics first (instant, zero cost), then an AI-assisted semantic check for attempts no fixed pattern anticipated. Degrades to heuristics-only if the AI call fails; never depends on AI availability to keep working. |
| **Classification** | Multi-label classification with confidence scores. Bulk mail that doesn't cleanly fit a category (unsubscribe links, "view in browser" banners) falls back to Newsletter rather than a dead-end Uncategorized, so it still feeds the knowledge pipeline. |
...
| **Decision** | Assigns one of five actions with a confidence score and a plain-language reason. |
...
1. **Create the instance.** In the Alibaba Cloud ECS console, launch an instance — Ubuntu 22.04 LTS, 2 vCPU / 4 GB RAM is comfortable for a single-instance deployment. Attach a public IP.
...
5. **Configure environment:**
   ...
   ```
6. **Run it under a process supervisor** (required for auto-restart, including in-app updates):
   ```bash
   npm install -g pm2
...
- Guardian: heuristic + AI-assisted phishing/injection/malware defense
- 8-label classification with a newsletter fallback for bulk mail
- Recency+importance+relevance memory with hierarchical consolidation
- Autonomous, policy-gated, round-capped negotiation (Track 3: Agent Society)
- Owner-initiated negotiation (define a goal, MailOS sends the opening move)
...
- Live-editable settings (Qwen key/model/endpoint, negotiation policy) — no restart
- In-app update checker against GitHub releases, one-click update, data untouched
- Embedded zero-setup storage, optional MongoDB; embedded event bus, optional Redis/BullMQ
...
- **Load-tested concurrency** — many simultaneous negotiations across many mailboxes
...
1. Create `src/connectors/YourConnector.js` extending `AgentBase` and implementing `ConnectorInterface` (`connect`, `disconnect`, `startMonitoring`, `sync`, `send`).
